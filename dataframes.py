...
print(dfff['A'].mean()) # vai ser impresso a media da coluna "A":
#1.5


print(dfff['A'].fillna(value=dfff['A'].mean()))#os campos com valores NaN na coluna A vao ser preenchidos com o valor da media:
#0    1.0
#1    2.0
#2    1.5
#Name: A, dtype: float64

print(dfff.ffill()) # vai preencher os valores de NaN com o ultimo valor proximo:
 #    A    B  C
#0  1.0  5.0  1
#1  2.0  5.0  2
#2  2.0  5.0  3
...
group = rp.groupby("Classe")
group.sum()
print(group)#Vai ser impresso:
#                   Nome  Venda
#Classe
#Júnior      JorgeCarlos    320
#Pleno   RobertaPatricia    464
#Sênior        BrunoVera    593

group = rp.groupby("Classe")
mean_result = group['Venda'].mean()
print(mean_result)#Vai ser impresso:
#Classe
#Júnior    160.0
#Pleno     232.0
#Sênior    296.5
...
mingroup = rp.groupby("Classe").min()
print(mingroup)#Vai ser impresso:
#            Nome  Venda
#Classe
#Júnior    Carlos    120
#Pleno   Patricia    124
...
maxgroup = rp.groupby("Classe").max()
print(maxgroup)#Vai ser impresso:
#           Nome  Venda
#Classe
#Júnior    Jorge    200
#Pleno   Roberta    340
...
print(pd.merge(esquerda, direita))
#  key   A   B   C   D
#0  K0  A0  B0  C0  D0
#1  K1  A1  B1  C1  D1
#2  K2  A2  B2  C2  D2
...
print(pd.merge(esquerda, direita, on=['key1', 'key2']))#Vai ser impresso:
#  key1 key2   A   B   C   D
#0   K0   K0  A0  B0  C0  D0
#1   K1   K0  A2  B2  C1  D1
#2   K1   K0  A2  B2  C2  D2

print(pd.merge(esquerda, direita, how='outer', on=['key1', 'key2']))#Vai ser impresso:
#  key1 key2    A    B    C    D
#0   K0   K0   A0   B0   C0   D0
...
print(pd.merge(esquerda, direita, how='left', on=['key1', 'key2']))#Vai ser impresso:
#  key1 key2   A   B    C    D
#0   K0   K0  A0  B0   C0   D0
#1   K0   K1  A1  B1  NaN  NaN
#2   K1   K0  A2  B2   C1   D1
...
print(pd.merge(esquerda, direita, how='right', on=['key1', 'key2']))#Vai ser impresso:
#  key1 key2    A    B   C   D
#0   K0   K0   A0   B0  C0  D0
#1   K1   K0   A2   B2  C1  D1
#2   K1   K0   A2   B2  C2  D2
...
print(esquerda.join(direita))#Vai ser impresso:
#     A   B    C    D
#K0  A0  B0   C0   D0
#K1  A1  B1  NaN  NaN
#K2  A2  B2   C2   D2